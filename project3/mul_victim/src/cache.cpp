/*
 *simple cache simulator(inclusive, victim)
 */
...
Cache::Cache(MemoryManager *manager, Policy policy, Cache *lowerCache, Cache *victim,
...
void Cache::loadBlockFromLowerLevel(uint32_t addr, uint32_t *cycles) {
  ...
  for (uint32_t i = blockAddrBegin; i < blockAddrBegin + blockSize; ++i) {
    if (this->lowerCache == nullptr) {
      ...
      if (this->victim == nullptr){ // L2
        b.data[i - blockAddrBegin] = this->lowerCache->getByte(i, cycles);
      }
      else{  // L1
        b.data[i - blockAddrBegin] = this->victim->getByte(i, cycles);
      }
    } 
...
void Cache::writeBlockToLowerLevel(Cache::Block &b) {
  ...
  if (this->lowerCache == nullptr) {//L3
    for (uint32_t i = 0; i < b.size; ++i) {
      ...
      if (this->victim){ //L1
        for (uint32_t i = 0; i < b.size; ++i) {
          this->victim->setByte(addrBegin + i, b.data[i]);
        }
      }
      for (uint32_t i = 0; i < b.size; ++i) {
