/*
 * simple cache simulator -- exclusive
 */

#include <cstdio>
#include <cstdlib>

#include "simulator/ex_cache.h"
...
void Cache::setByte(uint32_t addr, uint8_t val, uint32_t *cycles) {
  ...
  if ((blockId = this->getBlockId(addr)) != -1) {
    ...
    if (!this->writeBack) {
      uint32_t addrBegin = this->getAddr(this->blocks[blockId]);
      for(uint32_t i = 0; i < blocks[blockId].size; ++i){
          this->memory->setByteNoCache(addrBegin + i, blocks[blockId].data[i]);
      }  
      if(this->lowerCache){
          Cache *current = this;
          while(current->lowerCache != nullptr){
              current = current->lowerCache;
          }
          this->statistics.totalCycles += current->policy.missLatency;
      }
      else{
          this->statistics.totalCycles += this->policy.missLatency;
      }
...
void Cache::loadBlockFromLowerLevel(uint32_t addr, uint32_t *cycles) {
  ...
  for (uint32_t i = blockAddrBegin; i < blockAddrBegin + blockSize; ++i) {
    if (this->lowerCache == nullptr) { // load from memory
    ...
    // Invalidate the block in the lower cache after it's loaded
    if (this->lowerCache) {
        int i = this->lowerCache->getBlockId(addr);
        if(i != -1){
            this->lowerCache->blocks[i].valid = false;
        }
    }
...
void Cache::printStatistics() {
  ...
  printf("Num Hit: %d\n", this->statistics.numHit);
  printf("Num Miss: %d\n", this->statistics.numMiss);
  int a = this->statistics.numMiss;
  int b = this->statistics.numHit;
  printf("Miss rate: %.10f\n", (double)a/(a+b));
